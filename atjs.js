(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.atjs) {
      window.wwload.atjs = "wait";
      requirejs.config({
        paths: {
          "atjs": "libs/Atjs/js/jquery.atwho.min",// 示例: libs/qrcodejs/qrcode
          "caret": "libs/Atjs/js/jquery.caret.min"
        },
        shim: {
          "atjs": {
            deps: ["caret"]
      ...
      require(["atjs", "css!libs/Atjs/css/jquery.atwho.min"], function () {
        window.wwload.atjs = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.atjs === "wait") {
      setTimeout(function () {
   ...
   */
  function atwho($ele) {
    var atdata = eval($ele.attr("data--atdata") || ["Peter", "Tom", "Anne"]);
    var iframeid = $ele.attr("data-iframeid") || "#commframe"
    //console.log(typeof atdata);
    $ele.find(".note-editable").atwho('setIframe', $('iframe' + iframeid)[0], true);
    $ele.find(".note-editable").atwho({
      at: "@",
      data: atdata
    })
  }
  ...
  var evtInHandler = function ($ele, attribute, value) {
    switch (attribute) {
      case "data--atdata":
        $ele.find(".note-editable").atwho('destroy');
        if (value) {
          atwho($ele);
        }
        // 处理动作
